                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr class="text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            ...
                            <th class="py-2 pr-3">Status aplicado</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($types as $type)
                            <tr>
                                ...
                                <td class="py-2 pr-3 text-gray-600">{{ $statusOptions[$type->contract_status] ?? '—' }}</td>
                                <td class="py-2 pr-3 text-right space-x-3">
                                    <a href="{{ route('occurrence-types.edit', $type) }}" class="text-gray-600 hover:text-gray-900 text-sm">Editar</a>
                                    <form method="POST" action="{{ route('occurrence-types.destroy', $type) }}" class="inline" onsubmit="return confirm('Remover este tipo de ocorrência?');">
                                        @csrf
                                        @method('DELETE')
